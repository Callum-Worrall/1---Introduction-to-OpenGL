...
bool Particles::StartUp()
{
	...
	m_emitterControl->StartUp("Lighting Bar", 1280, 720, m_window);
	TwAddSeparator(m_emitterControl->GetBar(), "LIGHT BAR", "");
	TwAddVarRW(m_emitterControl->GetBar(), "Background Colour", TW_TYPE_COLOR4F, &m_background_color, "");
	//TwAddVarRW(m_emitterControl->GetBar(), "Specular Power", TW_TYPE_FLOAT,
	//							&m_specular_power, "group = Light min = 0 max = 100 step = 0.05");
